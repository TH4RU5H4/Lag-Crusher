...
export function JojoFxProvider({ children }: { children: ReactNode }) {
  ...
  const burst = useCallback((x: number, y: number, text?: string) => {
    ...
    setTimeout(() => setFx((prev) => prev.filter((f) => f.id !== id)), 1200);
  }, []);
  ...
  return (
    <FxContext.Provider value={{ burst, shake }}>
        ...
        {fx.map((f) => (
          <div key={f.id} className="absolute" style={{ left: f.x, top: f.y }}>
            <div className="absolute -translate-x-1/2 -translate-y-1/2">
              <div className="speedlines animate-burst h-40 w-40 rounded-full opacity-70" />
            </div>
            <div
